...
def analyze_with_groq(command_text, ref_url=""):
    ...
    try:
        resp = groq_client.chat.completions.create(model="llama-3.3-70b-versatile", messages=[{"role":"system","content":system_prompt},{"role":"user","content":msg}], temperature=0.1, max_tokens=300)
...
def build_filters(plan, w=854, h=480):
    ...
    if ep.get("sharpen"): f.append("unsharp=3:3:0.5")
    if ep.get("denoise"): f.append("hqdn3d=0.5:0.5:3:3")
    ...
    if cap:
        ...
        f.append(f"drawtext=text='{cap}':fontsize=20:fontcolor=white:fontstyle=bold:x=(w-text_w)/2:y=h-th-25:box=1:boxcolor=black@0.6:boxborderw=2")
    f.append("fade=t=in:st=0:d=0.1,fade=t=out:st=4.9:d=0.1")
    return ",".join(f)
...
def apply_edits(inp, out, plan, job_id):
    try:
        ...
        if not ok:
            jobs[job_id].update({"status":"error","error":"FFmpeg failed: "+str(err)[:250]})
            return
...
def upload():
    ...
    if not f.filename or not allowed_file(f.filename): return jsonify({"error":"Invalid file type"}), 400
...
def edit():
    ...
    if size_mb > 30: return jsonify({"error":"Free tier limit: videos must be <30MB. Yours is {:.0f}MB.".format(size_mb)}), 400
    jid = uuid.uuid4().hex
    ...
    def safe_worker():
        try: apply_edits(inp, out, plan, jid)
        except Exception as e: jobs[jid].update({"status":"error","error":str(e)[:200]})
...
def download(jid):
    ...
    if not fp.exists() or not fp.is_file(): return jsonify({"error":"Output missing"}), 500
